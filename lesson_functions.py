...
# *************************************************************************************************
# definition of a class to store previous pictures heat map
class heatmap():
    def __init__(self):
        self.old = []

# *************************************************************************************************
def combine_boxes(windows, image_shape, threshold=0, history=0):
    hot_windows = []
    image = None
    if len(windows)>0:
        # Create heatmap with windows
        heat = create_heatmap(windows, image_shape)
        # mix with previous heat map
        heatmap.old=cv2.addWeighted(heatmap.old,history,heat,1-history,0) # **********************
        heat=heatmap.old                                                   # *******************
        # apply threshold
        image= apply_threshold(heat,threshold)
